...
/**
 * The class is used for extracting database information
 * safely
 *
 * @author "Ivan Ivanov"
 *
...
public abstract class DataExtractor<T>
{
	...
	/**
	 * The method executes the given query
	 * and returns a non-null list of the given type
	 *
	 * @param inQueryName the name of the query
	 * to be executed. It must be a select query.
	 * @param inEntityManager the entity manager
	 * used for the database extraction operation
	...
	public List<T> execute(final String inQueryName,
		...
		final TypedQuery<T> query =
				inEntityManager.createNamedQuery(
						inQueryName, inClazz);
		prepareQuery(query);
