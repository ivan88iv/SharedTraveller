	...
	implements IRequestDAO
{
	...
	private static final String NULL_START_POINT =
			"The start point may not be null";

	private static final String NULL_END_POINT =
			"The end point may not be null";

	private static final String NULL_DEPARTURE_DATE =
			"The departure date may not be null";

	private static final String NULL_DRIVER_NAME =
			"The driver's user name may not be null";

	private static final String NULL_DRIVER =
			"The driver's user name may not be null.";
			...
			final String inDriverUsrname)
	{
		...
		DataExtractor<RequestEntity> extractor =
			...
			final Long inRequestId)
	{
				...
				new DataExtractor<RequestEntity>()
		{
			@Override
			protected void prepareQuery(
					final TypedQuery<RequestEntity> inQuery)
			{
				inQuery.setParameter("driver", inDriver)
					.setParameter("id", inRequestId);
			}
		};

		final List<RequestEntity> results =
				extractor.execute("Request.find", entityManager,
				RequestEntity.class,
				MessageFormat.format(FIND_REQUEST_BY_ID_ERROR,
						inRequestId, inDriver));
