...
public class RequestService implements Serializable
{
			...
			final Date inDepDate, final String inDriverUsrname)
	{
		...
		try
		{
			announcement = announcementDAO.loadAnnouncement(
					inStartPt, inEndPt, inDepDate, inDriverUsrname);
	 ...
	 * identification
	 *
	 * @param inRequestId the id of the request which status is to be
	 * changed
	 * @param inNewStatus the new status to change the request's status
	 * to
	 */
	public void changeStatus(final Long inRequestId,
