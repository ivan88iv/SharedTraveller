...
public class RestRequestService
{
	 ...
	 *
	 * @param inRequest the {@link RequestInfo} instance
	...
	public ResponseEntity<Void> sendTravelRequest(
		...
		if(null == inRequest)
		{
			throw new IncorrectInputException(
					NULL_NEW_REQUEST_EVENT, HttpStatus.NOT_FOUND);
		}
		...
		try {
			loadedAnnouncement = announcementService.loadAnnouncement(
					inRequest.getFromPoint(),
					inRequest.getToPoint(),
			...
			throw new InternalBusinessException(
					MessageFormat.format(TRAVEL_REQUEST_SEND_PROBLEM,
							inRequest), dee);
		}

		if(null == loadedAnnouncement || null == sender) {
			throw new IncorrectInputException(
					MessageFormat.format(INCORRECT_REQUEST_INPUT,
							inRequest), HttpStatus.NOT_FOUND);
		}
	...
	/**
	 * The method finds and returns all the request information
	 * that has been generated for the specified announcement
	 *
	 * @param inAnnouncement the announcement instance
	 * for which the request information is returned.
	 * @return the request information instances related to the
	 * specified announcement
	 */
	@RequestMapping(value = "/announcement/all", method = RequestMethod.GET,
			params = {"from", "to", "departureDate", "driver" })
	public ResponseEntity<List<? extends IRequestInfo>> getRequests(
			...
			final Date inDepartureDate,
			@RequestParam(value = "driver")
			final String inDriverUsername)
	{
		List<? extends IRequestInfo> requestInfos =
				new ArrayList<>();

		try
		{
			 requestInfos = requestService.loadRequests(
					inFrom, inTo, inDepartureDate, inDriverUsername);
		} catch(final InfoLookupException ile)
		{
			throw new InternalBusinessException(
					MessageFormat.format(REQUEST_EXTRACTION_PROBLEM,
							inFrom, inTo, inDepartureDate,
							inDriverUsername), ile);
		}

		return new ResponseEntity<List<? extends IRequestInfo>>(
				requestInfos, HttpStatus.OK);
	}

	/**
	 * The method rejects the provided request
	 *
	 * @param inRequestId the id of the request to be rejected
	 ...
	 *
	 * @param inRequestId the id of the request
	 * which status is to be changed
	 * @param inNewStatus the new status to use
	 * @return the response entity returned from the corresponding
	 ...
	 * to perform the status change
	 */
	private ResponseEntity<Void> changeRequestStatus(
